...
void SemanticAnalyzer::runMiniProg(const PNode* node, const std::string& code) {
...
void SemanticAnalyzer::call(const PNode* node, const std::string& func, const std::vector<std::string>& args) {
	PERR("CALL %s(...%llu):\n", func.data(), args.size());
	...
	if (func == "TABLE") {
		...
		for (const auto& c : tables.back().table) {
			PERR("name[%s] type[%s], value[%d], addr[%d], size[%d]\n",
				c.name.data(), (
				c.type == Sym::Type::Const ? "const" : (
					c.type == Sym::Type::Var ? "var" : "function"
					)
				), c.value, c.addr, c.size
			);
		}
		PERR("Pop!\n");
		tables.pop_back();
