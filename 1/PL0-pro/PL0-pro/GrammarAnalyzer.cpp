		...
		while (true) {
			...
			if (v.type == Type::Terminal) {
				if (v.value == code) {
					...
					PERR("Error T != code. At %s : %s\n", v.value.data(), code.data());
					return false;
				}
			} else {
				if (table.find({ v.value, code }) != table.end()) {
					...
					while (!helper.empty()) {
						...
						trstk.push_back(new TreeNode(&G.at(helper.top())));
						p->push(trstk[trstk.size() - 1]);
						helper.pop();
					}

				} else {
					PERR("Error table(NT, code) is empty. At %s : %s\n", v.value.data(), code.data());
					return false;
				}
			}
		}
